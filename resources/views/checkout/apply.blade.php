...
<head>
    ...
    <title>{{ __('checkout.apply_for_plan', ['plan' => ucfirst($plan->name)]) }}</title>
        ...
        <main class="container mx-auto px-6 py-12">
            <div class="max-w-2xl mx-auto">
                <div class="text-center mb-8">
                    <h1 class="text-3xl font-bold text-gray-800 mb-2">{{ __('checkout.apply_for_plan', ['plan' => ucfirst($plan->name)]) }}</h1>
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 mb-6">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-600">{{ __('checkout.starting_at') }} <span class="font-bold text-blue-600">&euro;{{ number_format($plan->price_monthly, 2) }}</span>{{ __('checkout.per_month') }}</p>
                            </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="text-sm text-amber-800">
                                ...
                                <ol class="list-decimal list-inside space-y-1 text-amber-700">
                                    <li>{{ __('checkout.apply_step_1_signup') }}</li>
                                    <li>{{ __('checkout.apply_step_2_signup') }}</li>
                                    <li>{{ __('checkout.apply_step_3_signup') }}</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('plan.apply.submit') }}" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" id="shop_name" name="shop_name" required
                                ...
                                placeholder="{{ __('checkout.shop_name_placeholder') }}" value="{{ old('shop_name') }}">
                            @error('shop_name')
                        ...
                        <div>
                            ...
                            <select id="language_code" name="language_code" required
                                ...
                                <option value="sq" {{ old('language_code') == 'sq' ? 'selected' : '' }}>Shqip (Albanian)</option>
                            </select>
                            @error('language_code')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="currency_code" name="currency_code" required
                                ...
                                <option value="MKD" {{ old('currency_code') == 'MKD' ? 'selected' : '' }}>MKD (ден)</option>
                            </select>
                            @error('currency_code')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <hr class="my-6">
                        <h3 class="text-lg font-medium text-gray-800">{{ __('checkout.contact_information') }}</h3>
                        ...
                        <button type="submit"
                            ...
                            {{ __('checkout.submit_application') }}
                        </button>
                ...
                <div class="text-center mt-6">
                    <p class="text-gray-600 mb-2">{{ __('checkout.want_instant_access') }}</p>
